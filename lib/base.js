...
// Accepts:
// @param moduleName the module name to listen with
// @param typeOfCallback open|receiver|provider|close
...
// });
//
Oneline.on = function(/**polymorph**/) {
   var makeSureCallback = typeof arguments[0] === 'string' && typeof arguments[1] === 'string' ?  true : false;
   var successCallback = typeof arguments[arguments.length-1] === 'function' ? arguments[arguments.length-1] : noop;
   var moduleTypeOrName = makeSureCallback ? arguments[0] : Oneline.currentModule;
   var typeofCallback =makeSureCallback ? arguments[1] : arguments[0];
   Oneline.log("Attaching callback for", { "module": moduleTypeOrName, "callback": successCallback});
   if(!makeSureCallback) { // good
      if (Oneline.isValidCallback(typeofCallback)) {
        if (typeof Oneline[typeofCallback+'Callback'] === 'undefined') {
        Oneline[typeofCallback+'Callback']=successCallback;
        } else {
        Oneline.log("You have already stored this callback"); 
        }
      } else {
        Oneline.log("Please provide open|close|receiver|provider as the typeofcallback");
      }
        
   } else {
      //var messageType = arguments[arguments.length-2];
      if(Oneline.isValidCallback(typeofCallback)) {
        if (typeof Oneline[typeofCallback+'Callbacks'][moduleTypeOrName] ==='undefined') {
          Oneline[typeofCallback+'Callbacks'][moduleTypeOrName]=successCallback;
        } else {
...
// is it a valid callback
// @param callbackname accepts either open,receiver,provider,and close
//
Oneline.isValidCallback = function(callbackname) {
   if (callbackname === 'open' ||
     callbackname === 'close' ||
    callbackname === 'provider' ||
    callbackname === 'receiver') {
     return true;
    }
   return false;
};
...
Oneline.listen = function(options,successCallback,errorCallback) {
   ...
   Oneline.currentModule = options.moduleName; 
   ...
   Oneline.connectWithMaster(options.moduleName, url, function(error,connection) {
     ...
     var conn = ws.createServer(function(conn) {
      ...
      conn.on("listening", function() {
        ...
        Oneline.successCallbacks[options.moduleName]();

      });
      conn.listen(options.brokerPort,options.brokerHost);
   }, function() {
